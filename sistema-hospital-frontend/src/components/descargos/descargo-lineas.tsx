...
export function DescargoLineas({ descargo, onUpdate }: DescargoLineasProps) {
  ...
  const [lineaServicioForm, setLineaServicioForm] =
    useState<LineaServicioFormData>({
      servicioId: 0,
      cantidad: 1,
    });

  const [lineaProductoForm, setLineaProductoForm] =
    useState<LineaProductoFormData>({
      productoId: 0,
      cantidad: 1,
    });
  ...
  const handleAddLineaServicio = async () => {
    if (!lineaServicioForm.servicioId || lineaServicioForm.cantidad <= 0) {
      toast.error("Error", {
        description:
          "Debe seleccionar un servicio y especificar una cantidad válida",
      });
      return;
    }
  ...
  const handleAddLineaProducto = async () => {
    if (!lineaProductoForm.productoId || lineaProductoForm.cantidad <= 0) {
      toast.error("Error", {
        description:
          "Debe seleccionar un producto y especificar una cantidad válida",
      });
      return;
    }
  ...
  // Separar las líneas por tipo
  const lineasServicio = (descargo.lineas || []).filter(
    (linea) => linea.servicio !== null
  );
  const lineasProducto = (descargo.lineas || []).filter(
    (linea) => linea.producto !== null
  );

  const calcularTotal = () => {
    return (descargo.lineas || []).reduce(
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-medium">
                {descargo.paciente?.documentoIdentidad}
              </p>
            </div>
            <div>
              <span className="text-muted-foreground">Líneas de Servicio:</span>
              <p className="font-medium">{lineasServicio.length}</p>
            </div>
            <div>
              <span className="text-muted-foreground">Líneas de Producto:</span>
              <p className="font-medium">{lineasProducto.length}</p>
            </div>
          </div>
        </CardContent>
      </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setLineaServicioForm((prev) => ({
                        ...prev,
                        servicioId: Number.parseInt(value),
                      }))
                    }
                ...
                <div className="flex items-end">
                  <Button
                    onClick={handleAddLineaServicio}
                    disabled={isAddingLinea}
                    className="w-full"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setLineaProductoForm((prev) => ({
                        ...prev,
                        productoId: Number.parseInt(value),
                      }))
                    }
                ...
                <div className="flex items-end">
                  <Button
                    onClick={handleAddLineaProducto}
                    disabled={isAddingLinea}
                    className="w-full"
                  >
                    <Plus className="mr-2 h-4 w-4" />
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Servicios ({lineasServicio.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Productos ({lineasProducto.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {/* Botón para pasar de estado pendiente -> descargado */}
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Wrench className="h-5 w-5" />
            Descargo #{descargo.nro}
            <Badge variant="outline">{formatPrice(calcularTotal())}</Badge>
            <div className="flex gap-2">
              <Button
                onClick={async () => {
                  await DocumentoService.descargarDescargo(descargo.id!);
                  onUpdate();
                }}
                disabled={isAddingLinea}
                className="w-full"
              ...
              <Button
                onClick={async () => {
                  const fac = await DocumentoService.facturarDescargo(
                    descargo.id!
                  );
                  window.location.href = `/facturas/${fac.id}`;
                }}
                disabled={isAddingLinea}
                className="w-full"
              >
