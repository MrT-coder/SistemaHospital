...
import type { Descargo, DescargoFormData, DescargoUpdateData } from "../types/descargo"
import { toast } from "sonner"
import { ArrowLeft, Server, AlertTriangle, RefreshCw, Eye, Edit } from "lucide-react"
...
export default function DescargosPage() {
  ...
  const [selectedDescargo, setSelectedDescargo] = useState<Descargo | undefined>()
  ...
  const loadDescargoDetails = async (id: number) => {
    try {
      console.log("Cargando detalles del descargo:", id)
      const descargo = await DescargoService.getById(id)
      console.log("Detalles cargados:", {
        id: descargo.id,
        lineasServicio: descargo.lineasServicio?.length || 0,
        lineasProducto: descargo.lineasProducto?.length || 0,
        valorTotal: descargo.valorTotal
      })
      ...
      toast.error("Error", {
        description: error.message || "No se pudo cargar los detalles del descargo",
      })
      return null
    }
  }
  ...
  const handleView = async (descargo: Descargo) => {
    const loadedDescargo = await loadDescargoDetails(descargo.id!)
    if (loadedDescargo) {
      setViewMode("view")
    }
  }
  ...
  const handleSubmit = async (data: DescargoFormData) => {
    ...
    try {
      if (viewMode === "create") {
        ...
        const updatedDescargo = await DescargoService.update(selectedDescargo.id, updateData)
        setDescargos((prev) => prev.map((d) => (d.id === selectedDescargo.id ? updatedDescargo : d)))
        toast.success("Éxito", {
      ...
      toast.error("Error", {
        description: error.message || `No se pudo ${viewMode === "create" ? "crear" : "actualizar"} el descargo`,
      })
    } finally {
    ...
    <div className="container mx-auto py-6 px-4">
          ...
          <AlertDescription>
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={loadDescargos} disabled={isLoading}>
                  {isLoading ? (
      ...
      {viewMode === "list" ? (
        ...
        <div className="space-y-4">
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Badge variant={getEstadoBadgeVariant(selectedDescargo.estado!)}>{selectedDescargo.estado}</Badge>
              </div>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <p className="font-medium">{selectedDescargo.paciente?.nombre}</p>
                </div>
                <div>
                  <span className="text-sm text-muted-foreground">Documento:</span>
                  <p className="font-medium">{selectedDescargo.paciente?.documentoIdentidad}</p>
                </div>
                <div>
                  <span className="text-sm text-muted-foreground">Fecha:</span>
                  <p className="font-medium">{new Date(selectedDescargo.fecha).toLocaleDateString("es-CO")}</p>
                </div>
                <div>
                  <span className="text-sm text-muted-foreground">Total:</span>
                  <p className="font-medium text-green-600">{formatPrice(selectedDescargo.valorTotal)}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          {/* Gestión de líneas */}
          <DescargoLineas descargo={selectedDescargo} onUpdate={handleUpdateDescargo} />
        </div>
      ) : (
        <div className="space-y-4">
