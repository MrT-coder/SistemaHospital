import type { Servicio, ServicioFormData, ServicioTipo } from "../types/servicio"
...
3. Prueba acceder directamente: ${url}`
  ...
  static async getAll(): Promise<Servicio[]> {
    ...
    const result = await this.makeRequest<Servicio[]>(url, {
      method: "GET",
    })
  ...
  static async getById(id: number): Promise<Servicio> {
    const url = `${API_BASE_URL}/api/servicios/${id}`
    const result = await this.makeRequest<Servicio>(url, {
      method: "GET",
    })
  ...
  static async getByTipo(tipo: ServicioTipo): Promise<Servicio[]> {
    const url = `${API_BASE_URL}/api/servicios/tipo/${tipo}`
    const result = await this.makeRequest<Servicio[]>(url, {
      method: "GET",
    })
  ...
  static async create(servicio: ServicioFormData): Promise<Servicio> {
    const url = `${API_BASE_URL}/api/servicios`
    const result = await this.makeRequest<Servicio>(url, {
      method: "POST",
  ...
  static async update(id: number, servicio: ServicioFormData): Promise<Servicio> {
    const url = `${API_BASE_URL}/api/servicios/${id}`
    const result = await this.makeRequest<Servicio>(url, {
      method: "PUT",
      body: JSON.stringify(servicio),
  ...
  static async testConnection(): Promise<{ success: boolean; message: string; details?: any }> {
    try {
      ...
      return {
        success: true,
        message: `Conexión exitosa. Se encontraron ${result.data?.length || 0} servicios.`,
        details: {
