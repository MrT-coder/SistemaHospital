import type { Producto, ProductoFormData, ProductoTipo } from "../types/producto"
...
3. Prueba acceder directamente: ${url}`
  ...
  static async getAll(): Promise<Producto[]> {
    ...
    const result = await this.makeRequest<Producto[]>(url, {
      method: "GET",
    })
  ...
  static async getByTipo(tipo: ProductoTipo): Promise<Producto[]> {
    const url = `${API_BASE_URL}/api/productos/tipo/${tipo}`
    const result = await this.makeRequest<Producto[]>(url, {
      method: "GET",
    })
  ...
  static async create(producto: ProductoFormData): Promise<Producto> {
    const url = `${API_BASE_URL}/api/productos`
    const result = await this.makeRequest<Producto>(url, {
      method: "POST",
  ...
  static async update(id: number, producto: ProductoFormData): Promise<Producto> {
    const url = `${API_BASE_URL}/api/productos/${id}`
    const result = await this.makeRequest<Producto>(url, {
      method: "PUT",
      body: JSON.stringify(producto),
  ...
  static async testConnection(): Promise<{ success: boolean; message: string; details?: any }> {
    try {
      ...
      return {
        success: true,
        message: `Conexión exitosa. Se encontraron ${result.data?.length || 0} productos.`,
        details: {
